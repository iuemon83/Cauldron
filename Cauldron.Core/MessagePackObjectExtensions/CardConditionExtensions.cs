    ...
    public static class CardConditionExtensions
    {
            ...
            Card effectOwnerCard, EffectEventArgs effectEventArgs, Card cardToMatch)
        {
            ...
            return
                ...
                && (await (_this.CostCondition?.IsMatch(cardToMatch.Cost, effectOwnerCard, effectEventArgs)
                    ?? ValueTask.FromResult(true)))
                && (await (_this.PowerCondition?.IsMatch(cardToMatch.Power, effectOwnerCard, effectEventArgs)
                    ?? ValueTask.FromResult(true)))
                && (await (_this.DiffPowerCondition?.IsMatch(cardToMatch.PowerBuff, effectOwnerCard, effectEventArgs)
                    ?? ValueTask.FromResult(true)))
                ...
                && (await (_this.ZoneCondition?.IsMatch(effectOwnerCard, effectEventArgs, cardToMatch.Zone)
                    ?? ValueTask.FromResult(true)))
                && (await (_this.PositionCondition?.IsMatch(cardToMatch, effectOwnerCard, effectEventArgs)
                    ?? ValueTask.FromResult(true)))
                ;
        }
    }
